...
class FemaleMatingModel():
    ...
    def generateFemale(self, size, fitness, ran, femaleSigma, femaleMu, flatcost, fitbase, memoryLength, mutationLambda):
        ...
        if(self.femaleType == 1):
            ...
            for x in range(size):
                female = FemaleThreshold(threshold = self.ran.norran(femaleSigma, femaleMu), mlambda = mutationLambda,fitness_function = ffunction, fitbase = fitbase,flatcost=flatcost)
                pop.append(female)
        return pop
